...
public class HomeController {
    ...
    Mono<Rendering> home(
        ...
        return Mono.just(Rendering.view("home.html")
                ...
                .modelAttribute("cart", inventoryService.getCart(cartName(oauth2User))
                        .defaultIfEmpty(Cart.from(cartName(oauth2User))))
    ...
    Mono<String> addToCart(@AuthenticationPrincipal OAuth2User oauth2User, @PathVariable String id) {
        return inventoryService.addItemToCart(cartName(oauth2User), id)
                .thenReturn("redirect:/");
    }

    @DeleteMapping("/remove/{id}")
    Mono<String> removeFromCart(@AuthenticationPrincipal OAuth2User oauth2User, @PathVariable String id) {
        return inventoryService.removeOneFromCart(cartName(oauth2User), id)
                .thenReturn("redirect:/");
    }

    private String cartName(OAuth2User oauth2User) {
